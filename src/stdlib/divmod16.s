...
.proc divmod16
    ...
    ;
    ; In:
    ;   ZP::word_a = the number to divide
    ;   ZP::word_b = what to divide by
    ; Out:
    ;   ZP::word_a = quotient
    ;   ZP::word_b = preserved
    ;   ZP::word_c = remainder
...
@divloop:
    ...
    rol ZP::word_a            ; Low byte << 1
    rol ZP::word_a + 1        ; High byte << 1
    rol ZP::word_c            ; Low byte << 1
    rol ZP::word_c + 1        ; High byte << 1
    ...
    lda ZP::word_c            ; Load low byte of remainder
    sbc ZP::word_b            ; Subtract low byte of divisor -> A
    tay                       ; A -> Y
    lda ZP::word_c + 1        ; Load high byte of remainder
    sbc ZP::word_b + 1        ; Subtract high byte of divisor -> A
    ...
    sty ZP::word_c            ; Update remainder low byte <- Y
    sta ZP::word_c + 1        ; Update remainder high byte <- A

@no_div_possible:
    dex
    bne @divloop              ; Process all 16 bits, before continuing.

    rol ZP::word_a            ; Shift last carry bit (representing if the last
    rol ZP::word_a + 1        ; division was possible or not) into the quotient.
